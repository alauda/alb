...
function check_alb_ingress_httpport() (
  ...
  for cluster in $(list-running-cluster | grep -v global); do
    ...
    if [[ "$ingressport" = "0" ]]; then
      if [[ "$(kubectl-with-cluster $cluster get ft -n cpaas-system $alb-11780 2>/dev/null | grep -c $alb-11780)" != "0" ]]; then
...
function check_alb_ingress_path_in_cluster() (
  local cluster=$1
  local count=$(kubectl-with-cluster $cluster get ingress -A -o yaml | grep -c "path:\s*/\s*$")
  if [[ "$count" -ne 0 ]]; then
...
function check_alb_ingress_rule_in_cluster() {
  ...
  local all_rule_source=$(kubectl-with-cluster $cluster get rule -n cpaas-system -l "alb2.cpaas.io/source-type=ingress" -o yaml | grep -E "^\s*source:$" -A 3)
  local all_rule_source_ingress_name=$(echo "$all_rule_source" | grep name: | awk '{print $2}' | uniq)
  local all_rule_label_ingress_name=$(kubectl-with-cluster $cluster get rule -n cpaas-system -l "alb2.cpaas.io/source-type=ingress" -o yaml | grep -E "^\s*alb2\.cpaas\.io/source-name:\s*" | awk '{print $2}' | awk -F '.' '{print $1}' | uniq)
  local all_ingress=$(kubectl-with-cluster $cluster get ingress -A | tail -n+2 | awk '{print $2}' | uniq)
...
function check_alb_hr_in_cluster() {
  ...
  while read -r hr; do
    local albname=${hr#"$cluster-"}
    if ! kubectl-with-cluster $cluster get alb2 -n cpaas-system $albname >/dev/null; then
