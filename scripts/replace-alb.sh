...
./scripts/build.sh
alb_tmux_index=local-pt:1.1
tmux send-keys -t $alb_tmux_index C-c
...
tmux send-keys -t $alb_tmux_index "md5sum ./alb/alb" Enter
sleep 2
...
tmux send-keys -t $alb_tmux_index "./run.sh" Enter