...
local function policy_with_otel(otel_ref, custom)
    ...
    return {
        ...
        config = {
            ...
            ["parent-base"] = {type = "otel", otel = {otel = {exporter = {collector = {address = default_collect_address, request_timeout = 1000}}, flags = {hide_upstream_attrs = false, trust_incoming_span = false},
                sampler = {
                    name = "parent_base", options = { parent_name = "always_off"},
                }
            }}},
            ["ratio-base"] = {type = "otel", otel = {otel = {exporter = {collector = {address = default_collect_address, request_timeout = 1000}}, flags = {hide_upstream_attrs = false, trust_incoming_span = false},
                sampler = {
...
function _M.test_parent()
    local res, id = _M.test_trace(policy_with_otel("parent-base"), {traceparent = "00-a0000000000000010000000000000001-0000000000000001-01"})
    h.assert_eq(res.status, 200)
    u.logs("trace id", id)
    u.logs("res ", res.status)
    local res, id = _M.test_trace(policy_with_otel("parent-base"), {traceparent = "00-b0000000000000010000000000000001-0000000000000002-00"})
...
function _M.test_ratio()
    ...
    local res, id = _M.test_trace(policy_with_otel("ratio-base"), {traceparent = "00-00000000000000010000000000000001-0000000000000001-01"})
    u.logs("trace id", id)
    u.logs("res ", res.status)
    h.assert_eq(res.status, 200)
    local res, id = _M.test_trace(policy_with_otel("ratio-base"), {traceparent = "00-ffffffff000000000000000100000001-0000000000000001-01"})
    u.logs("trace id", id)
    u.logs("res ", res.status)
    h.assert_eq(res.status, 404)
end
