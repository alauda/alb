...
function _M.test()
    require("policy_helper").set_policy_lua({
        certificate_map = {
            ["443"] = { cert = default_443_cert, key = default_443_key },
        },
        http = {
            tcp = { ["80"] = { { rule = "r1", internal_dsl = { { "STARTS_WITH", "URL", "/t1" } }, upstream = "test-upstream-1" } } }
        },
    ...
    h.assert_curl("https://127.0.0.1:1936/clear", { ssl_verify = false })
