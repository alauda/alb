...
var rulesResource = schema.GroupVersionResource{Group: "crd.alauda.io", Version: "v1", Resource: "rules"}

var rulesKind = schema.GroupVersionKind{Group: "crd.alauda.io", Version: "v1", Kind: "Rule"}

// Get takes name of the rule, and returns the corresponding rule object, and an error if there is any.
func (c *FakeRules) Get(ctx context.Context, name string, options v1.GetOptions) (result *alaudav1.Rule, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewGetAction(rulesResource, c.ns, name), &alaudav1.Rule{})

	if obj == nil {
		return nil, err
	}
	return obj.(*alaudav1.Rule), err
}

// List takes label and field selectors, and returns the list of Rules that match those selectors.
func (c *FakeRules) List(ctx context.Context, opts v1.ListOptions) (result *alaudav1.RuleList, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewListAction(rulesResource, rulesKind, c.ns, opts), &alaudav1.RuleList{})
	...
	list := &alaudav1.RuleList{ListMeta: obj.(*alaudav1.RuleList).ListMeta}
	for _, item := range obj.(*alaudav1.RuleList).Items {
...
func (c *FakeRules) Create(ctx context.Context, rule *alaudav1.Rule, opts v1.CreateOptions) (result *alaudav1.Rule, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewCreateAction(rulesResource, c.ns, rule), &alaudav1.Rule{})
...
func (c *FakeRules) Update(ctx context.Context, rule *alaudav1.Rule, opts v1.UpdateOptions) (result *alaudav1.Rule, err error) {
	obj, err := c.Fake.
		Invokes(testing.NewUpdateAction(rulesResource, c.ns, rule), &alaudav1.Rule{})
...
func (c *FakeRules) DeleteCollection(ctx context.Context, opts v1.DeleteOptions, listOpts v1.ListOptions) error {
	...
	_, err := c.Fake.Invokes(action, &alaudav1.RuleList{})
	return err
}

// Patch applies the patch and returns the patched rule.
func (c *FakeRules) Patch(ctx context.Context, name string, pt types.PatchType, data []byte, opts v1.PatchOptions, subresources ...string) (result *alaudav1.Rule, err error) {
	obj, err := c.Fake.
